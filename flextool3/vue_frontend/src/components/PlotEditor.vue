 ...
 */
async function updateParameterIndexes(
  projectId,
  analysisUrl,
  ...
  fetchResultParameterValueIndexes(
    ...
    for (const indexRecord of indexes) {
      ...
      for (const index of indexRecord.indexes) {
        ...
        for (const [class_name, parameter] of zip(
          indexRecord.class_names,
          indexRecord.parameter_names
        )) {
...
async function fetchData(
  ...
  const entityPromise = fetchResultEntities(projectId, analysisUrl, newClasses)
  const parameterPromise = fetchResultParameters(projectId, analysisUrl, newClasses)
...
export default {
  ...
  setup(props, context) {
    ...
    const selectionSelects = ref([
      makeEntityClassSelectionSelect(selectionOptions.get(entityClassKey), updateClassSelection)
    ])
