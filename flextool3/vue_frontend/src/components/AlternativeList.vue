...
function fetchAlternatives(projectId, modelUrl, alternativeList, state, errorMessage, emit) {
  Communication.fetchData('alternatives?', projectId, modelUrl)
    .then(function (data) {
...
function emitAvailableAlternativesChange(alternativeList, emit) {
  const availableAlternatives = new Set(
    alternativeList.value.map((alternative) => alternative.label)
  )
  ...
  setup(props, context) {
    ...
    const renameAlternative = function (renameData) {
      ...
      if (existing) {
        dialog.error({
          title: 'Cannot rename',
          content: 'An alternative with the same name already exists.'
        })
        return
      }
      const alternative = alternativeList.value.find(
        (item) => renameData.previousName === item.label
      )
    ...
    return {
      ...
      addAlternative(name) {
        ...
        alternativeList.value.splice(
          insertIndex >= 0 ? insertIndex : alternativeList.value.length,
          0,
          newAlternative
        )
