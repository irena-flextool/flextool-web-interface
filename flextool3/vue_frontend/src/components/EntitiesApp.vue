<template>
    <page name="Edit model" :index-url="indexUrl" :project-url="projectUrl" :edit-url="editUrl" :run-url="runUrl"
        ...
        <template #header>
            ...
            <commit-button :has-pending-changes="pendingChanges" :committing="committing"
                :error-message="commitErrorMessage" @commitRequest="commit" />
        </template>
        <n-layout id="main-layout" has-sider position="absolute">
            <n-layout-sider :width="siderWidth">
                <n-space>
                    <n-space vertical>
                        ...
                        <entity-list :project-id="projectId" :model-url="modelUrl" :class-id="classId"
                            :class-name="className" :class-type="classType" :inserted="insertedEntities"
                            @entity-select="setSelectedEntity" @entity-insert="storeEntityInsertion"
                            @entity-update="storeEntityUpdate" @entity-delete="storeEntityDeletion"
                            @relationships-clash="setRelationshipsClashErrorState"
                            @entity-dimensions-reveal="setEntityDimensions" />
                    </n-space>
                    <n-space vertical>
                        <n-h2>Alternatives</n-h2>
                        <alternative-selection :project-id="projectId" :model-url="modelUrl"
            ...
            <n-layout-content content-style="margin-left: 1em; margin-right: 1em">
                <n-grid cols="1 m:2" responsive="screen">
                    <n-grid-item>
                        <parameter-table :project-id="projectId" :model-url="modelUrl" :class-id="classId"
                            :entity-key="currentEntityKey" :diff="pendingModelChanges"
                            @open-value-editor-request="setValueEditorData"
                            @close-value-editor-request="possiblyClearValueEditor" @value-insert="storeValueInsertion"
                            @value-update="storeValueUpdate" @value-delete="storeValueDeletion" />
                    </n-grid-item>
                    <n-grid-item>
                        <indexed-value-editor :value-data="valueEditorData" :diff="pendingModelChanges"
