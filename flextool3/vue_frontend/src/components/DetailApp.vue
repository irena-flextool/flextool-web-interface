    ...
    <n-space>
      <n-space vertical>
        ...
        <n-space vertical>
          ...
          <n-p
            ><n-a :href="runUrl">Run</n-a> page allows you to set up scenarios and solve the
            model.</n-p
          >
        ...
        />
        <n-alert
          v-else-if="isDatabaseUploadSuccessful"
          title="Database upload successful"
          type="success"
        ></n-alert>
        ...
        <n-alert
          v-else-if="isExcelImportSuccessful"
          title=" Excel file import successful."
          type="success"
        >
      ...
      <n-space vertical>
        ...
        <n-p>
          Links on these pages can be opened in different browser tabs or windows. It is possible to
          e.g. open two Model editors side-by-side to compare or copy data around.
        </n-p>
...
export default {
  ...
  setup(props) {
    ...
    const isDatabaseUploadSuccessful = computed(
      () => uploadFileType.value === fileType.database && uploadState.value === state.done
    )
    const isImportingExcel = computed(
      () => uploadFileType.value === fileType.excel && uploadState.value === state.importing
    )
    const isAbortingExcelImport = computed(
      () => uploadFileType.value === fileType.excel && uploadState.value === state.aborting
    )
    const excelImportLog = ref([])
    const excelImportExecutionStatus = ref(executionStatus.yetToStart)
    const isExcelImportSuccessful = computed(
      () =>
        uploadState.value === state.done &&
        excelImportExecutionStatus.value === executionStatus.finished
    )
    const isExcelImportFailure = computed(
      () =>
        uploadFileType.value === fileType.excel &&
        uploadState.value === state.done &&
        excelImportExecutionStatus.value === executionStatus.error
    )
    const isExcelImportAborted = computed(
      () =>
        uploadFileType.value === fileType.excel &&
        uploadState.value === state.done &&
        excelImportExecutionStatus.value === executionStatus.aborted
    )
    const isExcelImportLogDialogShown = ref(false)
    const isAnotherExecutionOngoing = ref(false)
    const isUploadDisabled = computed(
      () =>
        isAnotherExecutionOngoing.value ||
        (uploadState.value !== state.idle &&
          uploadState.value !== state.done &&
          ...
          .then(function (data) {
            if (data.status !== 'busy') {
              followExecution(
                ...
                props.executionsUrl,
                excelImportLog,
                excelImportExecutionStatus,
                message,
                finishImportingExcel
              )
            } else {
    ...
    onMounted(function () {
      fetchCurrentExecution(props.projectId, props.executionsUrl)
        .then(function (data) {
          if (data.type !== executionType.importExcel && data.status === executionStatus.running) {
            ...
            data.type === executionType.importExcel &&
