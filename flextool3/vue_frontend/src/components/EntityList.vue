...
function fetchObjects(projectId, modelUrl, classId, entityList, state, errorMessage) {
  ...
  Communication.fetchData('objects?', projectId, modelUrl, extraBody)
...
function fetchAvailableObjects(projectId, modelUrl, classId) {
  ...
  return Communication.fetchData(
    'available relationship objects?',
    projectId,
    modelUrl,
    extraBody
  ).then(function (data) {
...
function fetchRelationships(
  ...
  const extraBody = { relationship_class_id: classId }
  Communication.fetchData('relationships?', projectId, modelUrl, extraBody)
    .then(async function (data) {
  ...
  setup(props, context) {
    ...
    const renameObject = function (renameData) {
      ...
      if (existing) {
        dialog.error({
          title: 'Cannot rename',
          content: 'An object with the same name already exists.'
        })
        return
      }
    ...
    return {
      ...
      emitEntitySelect(keys, selectedEntities) {
        ...
        context.emit('entitySelect', makeSelectedEntityInfo(selectedEntity))
      },
      addObject(name) {
        const existing = entityList.value.find(function (item) {
        ...
        if (existing) {
          dialog.error({
            title: 'Cannot create',
            content: 'An entity with the same name already exists.'
          })
          return
        }
        const newEntity = {
        ...
        entityList.value.splice(
          insertIndex >= 0 ? insertIndex : entityList.value.length,
          0,
          newEntity
        )
