...
export default {
  ...
  setup(props) {
    ...
    const isPlayButtonDisabled = computed(
      () => isExecuting.value || selectedScenarios.value.length === 0
    )
    ...
    onMounted(function () {
      const scenarioPromise = fetchData('scenarios?', props.projectId, props.modelUrl).then(
        function (data) {
    ...
    watch(logLines, function () {
      if (logInstance.value !== null) {
        nextTick(() =>
          logInstance.value.scrollTo({ top: logLines.value.length * 1000, slient: true })
        )
      }
    })
    return {
      ...
      execute: function () {
        ...
        executeExecution(props.projectId, props.executionsUrl, selectedScenarios.value)
          .then(function () {
            followExecution(
              props.projectId,
              props.executionsUrl,
